...
description: >
  Telegram channel plugin — auth, setup, and troubleshooting reference.
  This is the documentation for the Telegram core channel plugin at
  plugins/channels/telegram/. Triggers on "telegram channel", "telegram auth".
---

# Telegram Channel

Telegram channel plugin for NanoClaw. Lives at `plugins/channels/telegram/` and is loaded automatically by the plugin system when `TELEGRAM_BOT_TOKEN` is set.
...
### 3. Disable Group Privacy (for group chats)
...
Register using the `/add-channel` skill or via IPC `register_group` from the main group.
...
Pool bots are send-only (no polling). When a subagent calls `send_message` with a `sender` parameter, the host assigns a pool bot and renames it to match the sender's role. See `/add-telegram-swarm` for full setup guide.

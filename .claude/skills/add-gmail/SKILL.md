...
Configures Gmail access for agent containers using the `gog` CLI, the same tool used for Google Calendar. If the user already has Calendar set up via `/add-cal`, Gmail just needs the scope added.
...
**If gog is already installed and configured** (Calendar is set up via `/add-cal`):
...
**Only needed if `/add-cal` hasn't been run yet.**
...
```bash
cp -r .claude/skills/add-gmail/files/ plugins/gmail/
```

---
name: init-onecli
description: Install and initialize OneCLI Agent Vault for v1. Migrates existing .env credentials to the vault. Use after the Phase 3 OneCLI gateway commits land, or any time OneCLI needs to be set up from scratch.
---

# Initialize OneCLI Agent Vault

This skill installs OneCLI, configures the Agent Vault gateway, and migrates any existing `.env` credentials into it. Run this after the Phase 3 catch-up tasks B1+B2 bring in the OneCLI gateway integration, or any time OneCLI needs to be set up from scratch.
...
If `@onecli-sh/sdk` is NOT in package.json, the codebase hasn't been updated to use OneCLI yet. Tell the user: "OneCLI gateway commits aren't in this branch yet. Phase 3 catch-up tasks B1+B2 add the dep and wire `applyContainerConfig` into container-runner. Make sure those commits are present before running this skill, then retry." Stop here.
...
Parse the file for any of the credential variables listed above. (`ANTHROPIC_BASE_URL` is also read by v1 at startup but it is a config URL, not a credential — do not migrate it.)
...
**Important:** Only migrate credentials that containers use via outbound HTTPS. Channel tokens (`TELEGRAM_BOT_TOKEN`, `SLACK_BOT_TOKEN`, `SLACK_APP_TOKEN`, `DISCORD_BOT_TOKEN`) are used by the NanoTars host process to connect to messaging platforms — they must stay in `.env`.
...
```bash
tail -30 logs/nanotars.log | grep -i "onecli\|gateway"
```
...
**v1 stdin-pipe credentials still being honored when OneCLI should be active:** That's expected fallback behavior. v1's container-runner falls through to the legacy `.env` stdin pipe when OneCLI is unreachable. Once the gateway is up and `onecli secrets list` shows an Anthropic secret, the next container spawn picks up the OneCLI path. Existing running containers keep their original env until killed.

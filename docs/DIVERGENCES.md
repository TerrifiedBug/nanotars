...
| `package-lock.json` | Modified | OTHER | Permanent platform divergence — npm resolves different optional deps on Linux vs macOS (e.g., `@rollup/rollup-linux-x64-gnu` vs `darwin-arm64`). Accept and regenerate with `npm install` after upstream syncs |

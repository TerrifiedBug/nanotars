...
- [ ] **Bug — `pair-main` doesn't grant `owner` role to the first user.** The Phase 4 RBAC system (Apr 25) introduced `user_roles` gating for approval cards and sensitive actions, but the `pair-main` bootstrap flow (Apr 26, commit 10f2af5) only seeds `agent_groups[folder='main']` and allocates a pairing code — it never promotes the user who consumes the code to `owner`. Result: any fresh install ends up with zero owners and `requestApproval` returns `hasApprover: false` for every approval-card path (`add_mcp_server`, `install_packages`, `create_skill_plugin`, `create_agent`). Surfaced 2026-04-27 during slice 6 smoke test. Fix: in the pending-codes consumption path (or in `registerForIntent` when `intent='main'`), if the resulting user is the first to ever paired AND no `owner` role exists in `user_roles`, grant them `owner`. Defensive idempotent — multiple consumes of the same intent would still only grant once. ~1-2h including a regression test that runs pair-main + consume + asserts `listOwners()` returns the new user.

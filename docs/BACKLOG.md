...
- [ ] **Bug — AUTH preflight check returns false-negative when only `.credentials.json` is present.** Reported 2026-04-27 during WhatsApp install smoke test. The shared preflight in install-skill SKILL.md files runs `(grep -q "ANTHROPIC_API_KEY\|CLAUDE_CODE_OAUTH_TOKEN" .env 2>/dev/null || [ -f ~/.claude/.credentials.json ]) && echo "AUTH: ok" || echo "AUTH: missing"`. With `.credentials.json` present the second `-f` test should make the OR succeed, but `AUTH: missing` is reported anyway. Affected across multiple install skills (already noted in marketplace-PR backlog). Likely a tilde-expansion or precedence issue in the compound test. Investigation: reproduce on the host, isolate which clause fails. Fix: replace with explicit `[ -f "$HOME/.claude/.credentials.json" ]` and parenthesize correctly, OR split into two consecutive checks. Rolls into the marketplace skills sweep.

- [x] **Bug — WhatsApp auth flow fails on cold connect (both QR and pairing-code).** Closed 2026-04-27. Root cause: `plugins/channels/whatsapp/auth.js` was missing `fetchLatestWaWebVersion` + the `getPlatformId` patch that `index.js` already had (drift from upstream port). Baileys' bundled-default WA Web version was stale enough that WA returned HTTP 405 on the WS upgrade before QR/pairing-code display. Fix: ported both patches from `index.js` into `auth.js` (marketplace branch on `nanotars-skills`). Verified locally — clean auth state + `nanotars auth whatsapp` now produces a scannable QR.

- [x] **Feature — render QR code inline in Claude Code session for channel auth.** Closed 2026-04-27 alongside the auth-flow fix. Implementation: the auth script writes `data/channels/whatsapp/qr-data.png` (added `qrcode` dep to the plugin), and `add-channel-whatsapp/SKILL.md` instructs the agent to render the PNG via the multimodal Read tool. Replaces the macOS-only `open qr-auth.html` step. Same pattern (option 1 from the original brainstorm) extends to any future channel that needs QR auth — script writes a PNG, SKILL.md tells the agent to Read it.

- [ ] **`nanotars auth <channel>` for non-WhatsApp channels.** WhatsApp ships now (closed above). The dispatcher in `nanotars.sh` is generic — it shells out to `plugins/channels/<channel>/auth.js` if that file exists, so adding a new channel is just a matter of dropping an `auth.js` in the plugin. None of these are blocking — the channels work today via env-var configuration during `/add-channel-<X>` — but a unified `nanotars auth` surface is nicer UX once a few channels need it.
   - **Telegram** (`nanotars auth telegram`): bot-token-based; `auth.js` would prompt for `TELEGRAM_BOT_TOKEN`, validate via `getMe`, write to `.env`, optionally hit `setMyCommands`. Simpler than WhatsApp (no QR/socket). ~1h.
   - **Discord** (`nanotars auth discord`): bot-token-based + intents; similar to Telegram. ~1h once the channel plugin exists.
   - **Slack** (`nanotars auth slack`): OAuth flow; harder (requires HTTPS callback or socket-mode fallback). Defer until someone actually wants Slack interactively.
   - **Webhook** (`nanotars auth webhook`): N/A — webhook plugin has no auth surface beyond the URL it advertises.
   - Pattern to follow: the WhatsApp `auth.js` is a self-contained Node script that writes a status file + (where applicable) a renderable artefact. Keep that contract so the `add-channel-*` SKILL.md files share a poll-loop shape.

- [ ] **UX — `nanotars auth whatsapp` should short-circuit when already authenticated + service is running.** Reported 2026-04-28. Today `auth.js` always opens a Baileys connection. If creds already exist *and* the service is connected, WhatsApp Web's "one session per linked device" rule kicks one of the two off — the user sees `✓ Successfully authenticated` followed seconds later by `✗ Connection failed` (`stream:error / conflict / replaced`), and the service briefly disconnects + auto-reconnects. The end state is fine but the output is misleading and the brief drop is undesirable. Fix: at the top of `auth.js`, check whether `data/channels/whatsapp/auth/creds.json` (or the SQLite-backed auth state) is populated; if so, write `auth-status.txt = already_authenticated` and exit 0 without opening a socket. Add `--force` to wipe state and re-link. Telegram's future `auth.js` will inherit the same shape. ~30 min.

- [ ] **Feature — multi-account WhatsApp / Telegram support.** Reported 2026-04-28. Today each channel plugin assumes a single account: WhatsApp uses one shared `data/channels/whatsapp/auth/` directory and Telegram uses a single `TELEGRAM_BOT_TOKEN` slot in `.env`. Adding a second WhatsApp number or second Telegram bot per install requires either two installs or a hand-rolled split. Shape of the fix: namespace the auth/state per account (`data/channels/whatsapp/<label>/auth/`, `TELEGRAM_BOT_TOKENS=<label1>:<token1>,<label2>:<token2>`); extend `nanotars auth <channel> --account <label>`; add an `agent_groups` ↔ `<channel>:<account>` mapping at the entity-model layer so different groups route through different numbers. Telegram is the easier first cut (no QR/socket per-account). Half a day to a day for WhatsApp; ~half a day for Telegram. Defer until someone actually wants two numbers — until then, the single-account assumption is fine.

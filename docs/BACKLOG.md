...
- [ ] **Bug — WhatsApp auth flow fails on cold connect (both QR and pairing-code).** Reported 2026-04-27 during WhatsApp install smoke test. `node plugins/channels/whatsapp/auth.js` returns "Connection failed" before QR display; `--pairing-code --phone <num>` generates a code (e.g. `5RHL8LKP`) but the connection drops before the user can enter it. The skill notes "error 515 reconnect is normal" but this is a different failure mode — the WebSocket isn't establishing at all. Investigation: check Baileys version vs WhatsApp's current handshake; verify network egress from the host (some hosts block ws:// to mmg.whatsapp.net); check if the auth-state cleanup is racing the connection. Affects fresh installs only; once paired the channel stays connected. ~few hours of WA-specific debugging. May also resolve once the inline-QR-in-Claude-Code feature lands and the host-side pairing flow is more visible.

- [ ] **Feature — render QR code inline in Claude Code session for channel auth.** Reported 2026-04-27 during WhatsApp install smoke test. Today the QR-code auth flow for WhatsApp (and any future channel needing QR auth) requires the operator to either (a) run `node plugins/channels/whatsapp/auth.js` manually in a terminal — which fails with "Connection failed" in some environments — or (b) use the `--serve` HTTP fallback at `:8899`. Neither is great UX inside a Claude Code session. Investigate options: (1) Claude Code can render `image/png` content blocks inline via the agent's response — the auth script could write the QR to a temp PNG, the SKILL.md instructs the agent to read+render it; (2) a host-side endpoint that returns a Markdown image link Claude can fetch; (3) terminal sixel/iTerm protocol if running in a compatible terminal. Pairs with the existing auth-script's `data/channels/whatsapp/auth-status.txt` polling. ~half-day to prototype option (1). Affects WhatsApp + future Discord-bot-token / Slack-OAuth flows that benefit from interactive UI in chat-driven setup.

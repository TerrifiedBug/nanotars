...
Host-side hooks that read environment variables directly from the NanoTars process should declare those names in `hostEnvVars` instead. These variables are audited and documented, but they are not mounted into agent containers unless also listed in `containerEnvVars`.
